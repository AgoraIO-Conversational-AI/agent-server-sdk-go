...
│  ─────────         │  ────────────────               │
│  NewAgent()        │  NewOpenAI()                    │
│  NewAgentSession() │  NewElevenLabsTTS()             │
...
- **`agentkit.NewAgentSession`** — session lifecycle management (start, stop, say, interrupt, update)
...
|---|---|---|
| Build a conversational agent | `agentkit` | `NewAgent` -> `WithLlm` -> `WithTts` -> `NewAgentSession` -> `Start` |
